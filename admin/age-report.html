...
<script>
   async function load_report_by_age(){
      await fetch(BASE_URL + 'php/report_by_age.php')
      ...
      .then(response => {
         ...
         let keys = Object.keys(age_data);
         for(let id in keys){
            document.getElementById(keys[id]).innerText = age_data[keys[id]];
         }
