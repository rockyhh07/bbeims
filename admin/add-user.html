...
<script>
   function togglePass() {
      const checked = !document.getElementById("input-show-pass").checked;
      const showLabel = document.getElementById("show-pass");
      const hideLabel = document.getElementById("hide-pass");
   ...
   function add_user(e) {
      e.preventDefault();
      const data = document.getElementById("new-user-form");
      const form = new FormData(data);
      form.append("uid", USER_DATA.id);

      if(!validForm(form)) return;

      fetch(BASE_URL + 'php/user_new.php', {
         method: 'post',
         body: form
      })
         ...
         .then(response => {
            ...
            if (result[0].result) {
               addNotif("New User", "New user added successfully!", "g");
               data.reset();
               // window.location.href = `${BASE_URL}admin/manage-user.html`;
            } else {
               let msg = 'ERROR: \n';
               for (let err in result[0].error) msg += result[0].error[err] + '<br>';
               addNotif("Error occurred", msg, "r");
            }
         });
   }
   document.getElementById("new-user-save").addEventListener("click", add_user);
</script>