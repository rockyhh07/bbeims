<!DOCTYPE html>
<html lang="en">

<head>
   <meta charset="utf-8">
   <meta name="viewport" content="width=device-width, initial-scale=1">
   <title>Barangka Ibaba Evacuation Information Management System</title>
   <link rel="shortcut icon" href="../asset/img/logo.png" type="image/x-icon">
   <link rel="stylesheet" href="../asset/css/adminlte.min.css">
   <link rel="stylesheet" href="../asset/css/dashboard.css">
   <style type="text/css">
      td a.btn {
         font-size: 0.7rem;
      }

      td p {
         padding-left: 0.5rem !important;
      }

      th {
         padding: 1rem !important;
      }

      table tr td {
         padding: 0.3rem !important;
         font-size: 13px !important;
      }
   </style>
   <link rel="stylesheet" href="../asset/css/font.css">
   <style>
      table * {
         font-family: 'Poppins', sans-serif;
         font-size: 13px !important;
      }
   </style>
   <!-- Font Awesome -->
   <link rel="stylesheet" href="../asset/fontawesome/css/all.min.css">
   <script src="../asset/js/constants.js"></script>
   <script src="../asset/js/redirect_to_login.js"></script>
   <script src="../asset/js/alert.js"></script>
</head>

<body class="hold-transition sidebar-mini layout-fixed">
   <div class="wrapper">
      <!-- Navbar -->
      <nav class="main-header navbar navbar-expand navbar-white navbar-light"
         style="background-color: rgb(51, 99, 38);">
         <!-- Left navbar links -->
         <ul class="navbar-nav">
            <li class="nav-item">
               <a class="nav-link" data-widget="pushmenu" href="#" role="button"><i class="fas fa-bars"></i></a>
            </li>
         </ul>
         <!-- Right navbar links -->
         <ul class="navbar-nav ml-auto">
            <li class="nav-item">
               <a class="nav-link" data-widget="fullscreen" href="#" role="button">
                  <i class="fas fa-expand-arrows-alt"></i>
               </a>
            </li>
            <li class="nav-item">
               <a class="nav-link" onclick="logout()" href="#logout">
                  <i class="fas fa-power-off"></i>
               </a>
            </li>
         </ul>
      </nav>
      <!-- /.navbar -->
      <!-- Main Sidebar Container -->
      <aside class="main-sidebar sidebar-dark-primary elevation-4" style="background-color: rgb(68, 126, 52);">
         <!-- Brand Logo -->
         <a href="index.html" class="brand-link animated swing">
            <img src="../asset/img/logo2.png" alt="DSMS Logo" width="200"
               style="margin-top: -20px;margin-bottom: -60px;">
         </a>
         <!-- Sidebar -->
         <div class="sidebar">
            <!-- Sidebar Menu -->
            <nav class="mt-2">
               <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu"
                  data-accordion="false">
                  <li class="nav-item">
                     <a href="index.html" class="nav-link">
                        <i class="nav-icon fa fa-tachometer-alt"></i>
                        <p>
                           Dashboard
                        </p>
                     </a>
                  </li>
                  <li class="nav-item">
                     <a href="incident-type.html" class="nav-link">
                        <i class="nav-icon fa fa-globe-asia"></i>
                        <p>
                           Type of Incident
                        </p>
                     </a>
                  </li>


                  <li class="nav-item">
                     <a href="evacuation-center.html" class="nav-link">
                        <i class="nav-icon fa fa-hotel"></i>
                        <p>
                           Evacuation Center
                        </p>
                     </a>
                  </li>
                  <li class="nav-item">
                     <a href="#" class="nav-link">
                        <i class="nav-icon fas fa-address-card"></i>
                        <p>
                           Evacuee Information
                        </p>
                        <i class="right fas fa-angle-left"></i>
                     </a>
                     <ul class="nav nav-treeview">
                        <li class="nav-item">
                           <a href="add-evacuees.html" class="nav-link">
                              <i class="nav-icon fa fa-address-book"></i>
                              <p>Manage</p>
                           </a>
                        </li>
                        <li class="nav-item">
                           <a href="manage-evacuees.html" class="nav-link">
                              <i class="nav-icon fas fa-list-ol"></i>
                              <p>List</p>
                           </a>
                        </li>
                     </ul>
                  </li>


                  <li class="nav-item" id="user-management">
                     <a href="#" class="nav-link">
                        <i class="nav-icon fas fa-user-lock"></i>
                        <p>
                           Users
                        </p>
                        <i class="right fas fa-angle-left"></i>
                     </a>
                     <ul class="nav nav-treeview">
                        <li class="nav-item">
                           <a href="add-user.html" class="nav-link">
                              <i class="nav-icon fa fa-plus"></i>
                              <p>New</p>
                           </a>
                        </li>
                        <li class="nav-item">
                           <a href="manage-user.html" class="nav-link">
                              <i class="nav-icon fa fa-address-book"></i>
                              <p>Manage</p>
                           </a>
                        </li>
                     </ul>
                  </li>
                  <li class="nav-item">
                     <a href="#" class="nav-link">
                        <i class="nav-icon fas fa-chart-bar"></i>
                        <p>
                           Reports
                        </p>
                        <i class="right fas fa-angle-left"></i>
                     </a>
                     <ul class="nav nav-treeview">
                        <li class="nav-item">
                           <a href="evacuees-report.html" class="nav-link">
                              <i class="nav-icon fa fa-users"></i>
                              <p>Evacuees</p>
                           </a>
                        </li>
                        <li class="nav-item">
                           <a href="gender-report.html" class="nav-link">
                              <i class="nav-icon fa fa-venus-mars"></i>
                              <p>Evacuees by Gender</p>
                           </a>
                        </li>

                        <li class="nav-item">
                           <a href="age-report.html" class="nav-link">
                              <i class="nav-icon fa fa-sort-numeric-up-alt"></i>
                              <p>Evacuees by Age</p>
                           </a>
                        </li>
                        <li class="nav-item">
                           <a href="incident-report.html" class="nav-link">
                              <i class="nav-icon fa fa-globe-asia"></i>
                              <p>Evacuees by Incident</p>
                           </a>
                        </li>
                        <li class="nav-item">
                           <a href="center-report.html" class="nav-link">
                              <i class="nav-icon fa fa-hospital-alt"></i>
                              <p>Evacuees by Center</p>
                           </a>
                        </li>
                     </ul>
                  </li>
               </ul>
            </nav>
            <!-- /.sidebar-menu -->
         </div>
         <!-- /.sidebar -->
      </aside>
      <!-- Content Wrapper. Contains page content -->
      <div class="content-wrapper">
         <!-- Content Header (Page header) -->
         <div class="content-header">
            <div class="container-fluid">
               <div class="row mb-2">
                  <div class="col-sm-6">
                     <h1 class="m-0" style="color: rgb(232, 193, 33);"><span class="fa fa-address-card"></span> 
                        Evacuees</h1>
                  </div>
                  <!-- /.col -->
                  <div class="col-sm-6">
                     <ol class="breadcrumb float-sm-right">
                        <li class="breadcrumb-item"><a href=".">Home</a></li>
                        <li class="breadcrumb-item active">Evacuees</li>
                     </ol>
                  </div>
                  <!-- /.col -->
               </div>
               <!-- /.row -->
            </div>
            <!-- /.container-fluid -->
         </div>
         <!-- /.content-header -->
         <!-- Main content -->
         <section class="content">
            <div class="container-fluid">
               <div class="card card-info p-3">

                  <div class="col-md-12">
                     <div class="d-flex flex-row-reverse">
                        <button class="btn btn-primary" onclick="archive()">
                           <i class="fas fa-archive"></i> Archive
                        </button>
                     </div>
                     <table id="evacuee" class="table table-bordered table-striped">
                        <thead>
                           <tr>
                              <th>ID</th>
                              <th style="min-width: 225px;">Evacuee's Information</th>
                              <th>Address</th>
                              <th>Representative</th>
                              <th>Evacuation Center</th>
                              <th>Incident</th>
                              <th class="text-center">Action</th>
                           </tr>
                        </thead>
                        <tbody></tbody>
                     </table>
                     
                  </div>
               </div>
            </div>

            <!-- /.container-fluid -->
         </section>
         <!-- /.content -->
      </div>
      <!-- /.content-wrapper -->
   </div>
   <!-- ./wrapper -->
   <div id="delete" class="modal animated rubberBand delete-modal" role="dialog">
      <div class="modal-dialog modal-dialog-centered">
         <div class="modal-content">
            <div class="modal-body text-center">
               <img src="../asset/img/sent.png" alt="" width="50" height="46">
               <h3>Are you sure want to delete this evacuee?</h3>
               <div class="m-t-20"> <a href="#" class="btn btn-white" data-dismiss="modal">Close</a>
                  <button type="submit" class="btn btn-danger">Delete</button>
               </div>
            </div>
         </div>
      </div>
   </div>
   <!-- jQuery -->
   <script src="../asset/jquery/jquery.min.js"></script>
   <script src="../asset/js/bootstrap.bundle.min.js"></script>
   <script src="../asset/js/adminlte.js"></script>
   <!-- DataTables  & Plugins -->
   <script src="../asset/tables/datatables/jquery.dataTables.min.js"></script>
   <script src="../asset/tables/datatables-bs4/js/dataTables.bootstrap4.min.js"></script>
   <script src="../asset/tables/datatables-responsive/js/responsive.bootstrap4.min.js"></script>
   <script src="../asset/tables/datatables-buttons/js/buttons.bootstrap4.min.js"></script>
</body>

</html>

<script src="../asset/js/logout.js"></script>

<script>
   
   let evacCount = 0;

   async function archive() {

      if(evacCount == 0) {
         show_alert({
            title : `<i class="fas fa-exclamation-triangle"></i> Error Archiving`,
            body : "No Evacuees found",
            buttons : ["Okay"]
         });
         return;
      }

      await show_alert({
         title : `Archive data`,
         body : `Are you sure you want archive these evacuees?`,
         buttons : ["Yes", "No"]
      }, async function(ans) {
         if(!ans) return;

         const form = new FormData();
         form.append("uid", USER_DATA.id);
         await fetch(`${BASE_URL}php/archive.php`,{
            method : 'post',
            body : form
         })
         .then(response => response.json())
         .then(response => {
            show_alert({
               title : `Archived Successfully!`,
               body : `<b>${evacCount}</b> evacuess archived.`,
               buttons : ["Okay"]
            });
            evacCount = 0;
            load_all_evacuess();
         })
         .catch(err => console.error("ERROR: " + err));
      });

   }

   function delete_data(e) {
      const name = String($(e).data("name"));
      const id = String($(e).data("id"));
      show_alert({
         title : `<img src="${BASE_URL}asset/img/sent.png" > Delete record`,
         body : `Are you sure you want to delete <b>${name}</b>?`,
         buttons : ["Yes", "No"]
      }, function(ans) {
         console.log(ans)
      });
   }

   async function load_all_evacuess(){
      await fetch(BASE_URL + 'php/evacuee_get_all.php')
      .then(response => response.json())
      .then(response => {
         let result = response.result;
         if (!response) { console.error("Result not defined"); return; };

         if ($.fn.DataTable.isDataTable('#evacuee')) {
            $('#evacuee').DataTable().destroy();
            $('#evacuee').html("");
         }
         
         evacCount = result.length;

         const thead = `<thead><tr>
            <th>ID</th>
            <th style="min-width: 225px;">Evacuee's Information</th>
            <th>Address</th>
            <th>Representative</th>
            <th>Evacuation Center</th>
            <th>Incident</th>
            <th class="text-center">Action</th>
         </tr></thead>`;

         let tbody = `<tbody>`;
         for(let row of result){
            let fname = row.fname;
            let lname = row.lname;   
            let mname = row.mname;

            let name =  (fname && lname) ? `${lname}, ${fname} ${mname}` :
                        (fname) ? `${fname}` : 
                        (lname) ? `${lname}` : mname;
            
            let rep_fname = row.rep_fname;
            let rep_lname = row.rep_lname;   
            let rep_mname = row.rep_mname;

            let repName = (rep_fname && rep_lname) ? `${rep_lname}, ${rep_fname} ${rep_mname}` :
                        (rep_fname) ? `${rep_fname}` : 
                        (rep_lname) ? `${rep_lname}` : rep_mname;

            let age = row.birthday ? getAge(row.birthday) : "0";

            let id = (row.id == row.representative) ? 
                        "H-"+row.id.padStart(6, "0") : 
                        "M-"+row.id.padStart(6, "0")

            let cs= (row.civil_status == 'S') ? "SINGLE" :
                    (row.civil_status == 'M') ? "MARRIED" :
                    (row.civil_status == 'SP') ? "SEPARATED" :
                    (row.civil_status == 'W') ? "WIDOWED" : "--";
 
            tbody += `<tr>`;
            tbody += `<td style="white-space: nowrap;">${id}</td>`;
            tbody += `<td>
                  <p class="info"><b>Name:</b>${name}</p>
                  <p class="info"><b>Age:</b>${age}</p>
                  <p class="info"><b>Gender:</b>${row.gender}</p>
                  <p class="info"><b>Contact:</b>${row.contact}</p>
                  <p class="info"><b>Civil status:</b>${cs}</p>
               </td>`;
            tbody += `<td>${row.address}</td>`;
            tbody += `<td>
                  ${repName}
               </td>`;
            tbody += `<td>${row.evac_name}</td>`;
            tbody += `<td>
                  <div>
                     <b>Type: </b>${row.incident_name}
                  </div>
                  <div>
                     <b>Date: </b>${row.incident_date}
                  </div>
               </td>`;
            tbody += `<td class="text-center">
                  <button class="btn btn-sm btn-success" onclick="editData({
                     rep_id : '${row.id}',
                     incident_date : '${row.incident_date}',
                     evac_id : '${row.evac_id}',
                     incident_name : '${row.incident_name}',
                     representative : '${repName}',
                  })"><i class="fa fa-edit"></i> Edit</button>
                </td>`;
            tbody += `</tr>`;
         }

         tbody += `</tbody>`;
         $("#evacuee").html(thead + tbody);
         $("#evacuee").DataTable();
      });
   }

   async function editData(body = {}) {
      show_alert({
         title : "Edit Incident",
         body : `<div class="d-flex flex-column" style="gap: .5rem;">
               <div class="input-group">
                  <span class="input-group-text" id="evac_name">Family of</span>
                  <input class="form-control" value="${body.representative}" readonly>
               </div>
               <div class="input-group">
                  <span class="input-group-text" id="evac_name">Incident</span>
                  <input class="form-control" value="${body.incident_name}" readonly>
               </div>
               <div class="input-group">
                  <span class="input-group-text" id="incident_date">Incident Date</span>
                  <input id="update-incident_date" value="${body.incident_date}" type="datetime-local" class="form-control" aria-describedby="incident_date">
               </div>
               <div class="input-group">
                  <span class="input-group-text" id="evac_id">Evacuation Center</span>
                  <select class="form-control" id="update-evac_id" aria-describedby="evac_id">
                  </select>
               </div>
               <div style="font-size: small;">
                  <b>Note</b>: All members from the family of <b>${body.representative}</b> will be updated as well.
               </div>
            </div>`,
         buttons : ["Save", "Cancel"]
      }, async function(ans){
         if(!ans) return;

         const form = new FormData();
         form.append("uid", USER_DATA.id);
         form.append("representative", body.rep_id);
         form.append("incident_date", $("#update-incident_date").val());
         form.append("evac_id", $("#update-evac_id").val());

         await fetch(`${BASE_URL}php/evacuee_update_incident.php`, {
            method : 'post',
            body : form
         })
         .then(response => response.json())
         .then(response => {
            load_all_evacuess();
         })
         .catch(err => console.error("ERROR: " + err));
      });

      await fetch(`${BASE_URL}php/evac_center_get_all.php`)
      .then(response => response.json())
      .then(response => {
         const result = response.result;

         let options = '';
         for(row of result) {
            options += `<option value="${row.id}">${row.name}</option>`;
         }

         $("#update-evac_id").html(options);
         $("#update-evac_id").val(body.evac_id)
      })
   }

   load_all_evacuess();
</script>